...
export default class LoginScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(width / 2, 240, '[ Créer un compte ]', { fontSize: '20px', color: '#00ff88' })
      .setOrigin(0.5)
      .setInteractive({ useHandCursor: true })
      .on('pointerup', async () => { await this.handleRegister(); });

    this.add.text(width / 2, 280, '[ Se connecter ]', { fontSize: '20px', color: '#00ccff' })
      .setOrigin(0.5)
      .setInteractive({ useHandCursor: true })
      .on('pointerup', async () => { await this.handleLogin(); });

    this.add.text(width / 2, 340, '[ Continuer → Création de brute ]', { fontSize: '20px', color: '#ffffff' })
      .setOrigin(0.5)
      .setInteractive({ useHandCursor: true })
      .on('pointerup', () => {
        const token = localStorage.getItem('labrute_token');
        if (!token) { alert("Tu dois te connecter d'abord."); return; }
        this.scene.start('CharacterCreateScene');
      });

    // Nettoyage défensif: éviter toute boucle avec un ancien token invalide
