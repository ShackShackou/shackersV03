...
export default class MyShackersScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(width/2, 80, 'My SHACKERS', { fontSize: '36px', color: '#ffffff' }).setOrigin(0.5);

    this.add.text(width/2, 120, '[ Refresh list ]', { fontSize: '18px', color: '#00ccff' })
      .setOrigin(0.5)
      .setInteractive({ useHandCursor: true })
      .on('pointerup', () => this.fetchShackers());

    this.add.text(width/2, 160, '[ Create a SHACKER ]', { fontSize: '18px', color: '#00ff88' })
      .setOrigin(0.5)
      .setInteractive({ useHandCursor: true })
      .on('pointerup', () => this.scene.start('CharacterCreateScene'));

    this.listText = this.add.text(80, 210, 'Loading...', { fontSize: '18px', color: '#ffffaa' }).setOrigin(0,0);

    this.add.text(width/2, 520, '[ Fight! ]', { fontSize: '24px', color: '#ff6688' })
      .setOrigin(0.5)
      .setInteractive({ useHandCursor: true })
  ...
  private renderList() {
    ...
    if (this.shackers.length === 0) {
      this.listText.setText('No SHACKERS. Create one.');
      return;
    }
    ...
    this.input.on('pointerup', (pointer: Phaser.Input.Pointer) => {
      ...
      if (index >= 0 && index < this.shackers.length) {
        ...
        this.resultText?.setText(`Selection: A=${mark(this.shackerAId)} B=${mark(this.shackerBId)}`);
      }
    });
  }
  ...
  private async startFight() {
    ...
    if (!this.shackerAId || !this.shackerBId || this.shackerAId === this.shackerBId) {
      alert('Choose two different SHACKERS (click two lines).');
      return;
    }
